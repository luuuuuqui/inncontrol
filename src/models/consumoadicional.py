...
class ConsumoAdicional:
    ...
    def set_id_consumo(self, id_consumo: int) -> None: 
        if id_consumo <= 0: raise ValueError("ID do consumo deve ser um inteiro positivo.")
    ...
    def set_quantidade(self, quantidade: int) -> None:
        if quantidade < 0: raise ValueError("Quantidade do consumo não pode ser negativa.")
        self._quantidade = quantidade

    def set_data_consumo(self, data_consumo: datetime) -> None: 
        if data_consumo > datetime.now(): raise ValueError("Data do consumo não pode ser no futuro.")
